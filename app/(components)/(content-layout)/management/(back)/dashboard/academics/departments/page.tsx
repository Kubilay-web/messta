...
const PATH = "/management/dashboard/academics/departments";
...
export default async function DepartmentsPage() {
  ...
  // GET — tüm departmanlar + danışman sayısı
  ...
  async function createDept(data: { name: string; managerName: string; budget: string; budgetYear: string }) {
    "use server";
    try {
  ...
  async function updateDept(id: string, data: { name: string; managerName: string; budget: string; budgetYear: string }) {
    "use server";
    try {
      const name = data.name.trim();
      if (!name) return { ok: false, error: "Departman adı zorunlu." };
