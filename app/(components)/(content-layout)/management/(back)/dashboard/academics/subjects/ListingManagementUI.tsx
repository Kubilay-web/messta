...
import { Plus, Pencil, Trash2, X, FileText, Eye, TrendingUp } from "lucide-react";
...
const STATUS_LABEL: Record<string, string> = { ACTIVE: "Aktif", SOLD: "Satıldı", RENTED: "Kiralandı" };
                    ...
                    <tbody className="divide-y">
                      {listings.map((l) => (
                        <tr key={l.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                          ...
                          <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{l.agentName}</td>
                          <td className="px-4 py-3 text-gray-400 text-xs">{l.views}</td>
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-end gap-1">
                              <button onClick={() => openEdit(l)} className="p-1.5 rounded hover:bg-blue-50 dark:hover:bg-blue-900/30 text-blue-600 transition-colors" title="Düzenle">
                                <Pencil className="h-4 w-4" />
                              </button>
                              <button onClick={() => handleDelete(l.id)} disabled={deleting === l.id} className="p-1.5 rounded hover:bg-red-50 dark:hover:bg-red-900/30 text-red-500 transition-colors disabled:opacity-40" title="Sil">
                    ...
                    <div key={l.id} className="p-4 space-y-2">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="flex gap-1 shrink-0">
                          ...
                          <button onClick={() => handleDelete(l.id)} disabled={deleting === l.id} className="p-1.5 rounded hover:bg-red-50 text-red-500 disabled:opacity-40"><Trash2 className="h-4 w-4" /></button>
                        </div>
                      </div>
                      <div className="flex flex-wrap gap-2 items-center">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLOR[l.status] ?? ""}`}>{STATUS_LABEL[l.status] ?? l.status}</span>
                        <span className="text-xs text-gray-500">{TYPE_LABEL[l.listingType]}</span>
                        <span className="text-xs font-medium">{fmt(l.askingPrice, l.currency)}</span>
                        <span className="text-xs text-gray-400 flex items-center gap-0.5"><Eye className="h-3 w-3" />{l.views}</span>
                      </div>
        ...
        <Card className="border-t-4 border-blue-600 shadow-sm">
          <CardContent className="p-4 sm:p-6">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              {/* Mülk + Danışman — sadece yeni ilanlarda */}
              {!editing && (
              ...
              {editing && (
                <div className="space-y-1">
                  <label className={labelCls}>Sorumlu Danışman Adı</label>
