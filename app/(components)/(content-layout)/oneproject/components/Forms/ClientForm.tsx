...
import { PropertyClient as PrismaPropertyClient } from "@prisma/client";
...
type Props = {
  ...
  agencyName: string;
  editingId?: string;
  initialData?: Partial<PrismaPropertyClient> | null;
};
...
export default function ClientForm({
  ...
  const [selectedGender, setSelectedGender] = useState<any>(
    genderOptions.find((o) => o.value === initialData?.gender) ??
      genderOptions[0]
  );
  const [selectedContact, setSelectedContact] = useState<any>(
    contactOptions.find((o) => o.value === initialData?.contactMethod) ??
      contactOptions[0]
  );
  const [selectedCurrency, setSelectedCurrency] = useState<any>(
    currencyOptions.find((o) => o.value === initialData?.currency) ??
      currencyOptions[0]
  );
  const [selectedTypes, setSelectedTypes] = useState<string[]>(
    (initialData?.preferredPropertyTypes as string[]) ?? []
  );
  ...
  const {
    ...
    defaultValues: {
      ...
      preferredCities: Array.isArray(initialData?.preferredCities)
        ? (initialData!.preferredCities as string[]).join(", ")
        : ((initialData?.preferredCities as any) ?? ""),
  ...
  async function onSubmit(data: ClientProps) {
    ...
    data.minBudget = data.minBudget
      ? parseFloat(String(data.minBudget))
      : undefined;
    data.maxBudget = data.maxBudget
      ? parseFloat(String(data.maxBudget))
      : undefined;
    data.preferredCities =
      typeof data.preferredCities === "string"
        ? (data.preferredCities as string)
            .split(",")
            .map((c) => c.trim())
            .filter(Boolean)
        : data.preferredCities;
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <fieldset className="border rounded-lg p-4 space-y-4">
        <legend className="text-sm font-semibold px-1 text-gray-600">
          Kişisel Bilgiler
        </legend>
