...
// PUT    /api/portal/parent/property/[id]/visit?vid= → iptal / feedback
// DELETE /api/portal/parent/property/[id]/visit?vid= → iptal edilen geziyi sil
...
function money(n: number, cur: string) { return n.toLocaleString("tr-TR") + " " + cur; }
function fmt(d: string) { return format(new Date(d), "d MMM yyyy HH:mm", { locale: tr }); }

// Datetime-local input için minimum değer (şu anki zaman)
...
export default function ListingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // PUT visit — iptal (sadece SCHEDULED)
  ...
  // PUT visit — feedback (sadece COMPLETED)
  ...
  if (!listing) return (
    <div className="flex h-[60vh] flex-col items-center justify-center gap-1 text-gray-400">
      <p>İlan bulunamadı</p>
              ...
              <div className="text-right">
                {listing.monthlyRent ? (
                  <p className="text-xl font-bold text-green-600">
                    {money(listing.monthlyRent, listing.currency)}
        ...
        <Tabs defaultValue="details">
          <TabsList className="mb-4 w-full sm:w-auto">
            ...
            <TabsTrigger value="visits">
              Geziler {visits.length > 0 && (
                <span className="ml-1 rounded-full bg-blue-100 px-1.5 text-xs text-blue-700">{visits.length}</span>
              )}
            </TabsTrigger>
      ...
      <Dialog open={!!fbVisit} onOpenChange={(o) => { if (!o) setFbVisit(null); }}>
        <DialogContent className="sm:max-w-sm bg-white text-black">
          ...
          <div className="grid gap-3 py-4">
            ...
            <Textarea
              placeholder="Gezi hakkında düşünceleriniz…"
          ...
          <DialogFooter>
            ...
            <Button onClick={submitFeedback} disabled={saving || !fbText.trim()}>
